...
TypeId MtuBulkSendApplication::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::MtuBulkSendApplication")
            .SetParent<Application>()
            .SetGroupName("mtu")
            .AddConstructor<MtuBulkSendApplication>()
            .AddAttribute("SendSize", "The amount of data to send each time.", UintegerValue(512), MakeUintegerAccessor(&MtuBulkSendApplication::m_sendSize),
                          MakeUintegerChecker<uint32_t>(1))
            .AddAttribute("Remote", "The address of the destination", AddressValue(), MakeAddressAccessor(&MtuBulkSendApplication::m_peer), MakeAddressChecker())
            .AddAttribute("MaxBytes",
                          "The total number of bytes to send. "
                          "Once these bytes are sent, "
                          "no data  is sent again. The value zero means "
                          "that there is no limit.",
                          UintegerValue(0), MakeUintegerAccessor(&MtuBulkSendApplication::m_maxBytes), MakeUintegerChecker<uint32_t>())
            .AddAttribute("Protocol", "The type of protocol to use.", TypeIdValue(TcpSocketFactory::GetTypeId()), MakeTypeIdAccessor(&MtuBulkSendApplication::m_tid),
                          MakeTypeIdChecker())
            .AddAttribute("FlowLevelSize", "distinguish flow size", UintegerValue(0), MakeUintegerAccessor(&MtuBulkSendApplication::m_flowSizeLevel), MakeTypeIdChecker())
            .AddTraceSource("Tx", "A new packet is created and is sent", MakeTraceSourceAccessor(&MtuBulkSendApplication::m_txTrace), "ns3::Packet::TracedCallback");

    return tid;
}

MtuBulkSendApplication::MtuBulkSendApplication() : m_socket(0), m_connected(false), m_totBytes(0), m_priority(0) { NS_LOG_FUNCTION(this); }

MtuBulkSendApplication::~MtuBulkSendApplication() { NS_LOG_FUNCTION(this); }
...
Ptr<Socket> MtuBulkSendApplication::GetSocket(void) const { return m_socket; }

void MtuBulkSendApplication::SetPriority(uint32_t priority) { m_priority = priority; }

uint32_t MtuBulkSendApplication::GetPriority() const { return m_priority; }

void MtuBulkSendApplication::SetFlowSizeLevel(uint32_t flowSizeLevel) { m_flowSizeLevel = flowSizeLevel; }

uint32_t MtuBulkSendApplication::GetFlowSizeLevel(void) const { return m_flowSizeLevel; }

void MtuBulkSendApplication::SetSequenceNumber(int flowCount) { seq_num = flowCount; }

int MtuBulkSendApplication::GetSequenceNumber() const { return seq_num; }

void MtuBulkSendApplication::SetSegmentSize(uint32_t segSize) { m_segmentSize = segSize; }

uint32_t MtuBulkSendApplication::GetSegmentSize(void) { return m_segmentSize; }

void MtuBulkSendApplication::SetNdBandwidth(uint64_t bandwidth) { m_bandwidth = bandwidth; }

uint64_t MtuBulkSendApplication::GetNdBandwidth() { return m_bandwidth; }
...
// static void bufferTrace(uint32_t oldValue, uint32_t newValue) { std::cout << "im buffer" << newValue << std::endl; }
...
void MtuBulkSendApplication::StartApplication(void) {
    ...
    if (!m_socket) {
        ...
        m_socket->SetConnectCallback(MakeCallback(&MtuBulkSendApplication::ConnectionSucceeded, this), MakeCallback(&MtuBulkSendApplication::ConnectionFailed, this));
        m_socket->SetSendCallback(MakeCallback(&MtuBulkSendApplication::DataSend, this));
    }
...
void MtuBulkSendApplication::updateMTUandPriInDc(uint64_t bandwidth, double delay_prop, double delay_process, double delay_tx, double delay_rx) {
    ...
    // std::cout << "updateMTUandPriInDc computing, flowsize is " << m_maxBytes << "remain bytes: " << m_maxBytes - m_totBytes << std::endl;
