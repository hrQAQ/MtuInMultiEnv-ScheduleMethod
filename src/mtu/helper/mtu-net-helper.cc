...
ApplicationContainer MtuNetHelper::InstallApplication(Ptr<Node> sendNode, Ptr<Node> sinkNode, Ipv4Address remoteAddress, uint16_t port, uint32_t maxBytes, uint32_t sendSize,
...
void MtuNetHelper::InstallAllApplicationsInWAN(NodeContainer fromServers, NodeContainer destServers, double requestRate, struct cdf_table *cdfTable,
                                               std::vector<Ipv4Address> destAddress, uint32_t &flowCount, int port_start, int port_end, double timesim_start, double timesim_end,
                                               double time_flow_launch_end, uint64_t bandwidth, int numOfSwitches, double delay_prop, double delay_process, double delay_tx,
                                               double delay_rx) {
    int src_leaf_node_count = fromServers.GetN();
    int dst_leaf_node_count = destServers.GetN();
    uint16_t port = port_start;
...
void MtuNetHelper::InstallAllApplicationsInMix(NodeContainer fromServers, NodeContainer destServers, double requestRate, struct cdf_table *cdfTable,
                                               std::vector<Ipv4Address> destAddress, uint32_t &flowCount, int port_start, int port_end, double timesim_start, double timesim_end,
                                               double time_flow_launch_end, uint64_t bandwidth_dc, uint64_t bandwidth_wan, double delay_prop_dc, double delay_prop_wan,
                                               double delay_process_wan, double delay_process_dc, int numOfSwitches, double delay_tx, double delay_rx, std::string src_type) {
    int src_leaf_node_count = fromServers.GetN();
    int dst_leaf_node_count = destServers.GetN();
    uint16_t port = port_start;
    for (int i = 0; i < src_leaf_node_count; i++) {
        ...
        while (startTime < time_flow_launch_end) {
            ...
            int size = 1460;
            MtuDecision *md = new MtuDecision();
            int bestMtu = md->FindBestMtuInMix(flowSize, numOfSwitches, bandwidth_dc, bandwidth_wan, delay_prop_dc, delay_prop_wan, delay_process_dc, delay_process_wan, delay_tx,
                                               delay_rx, src_type);
            size = bestMtu - 40;

            ApplicationContainer applications;
            applications = InstallApplication(fromServers.Get(i), destServers.Get(destIndex), destAddress[destIndex], port, flowSize, size, priority, flowCount, bandwidth_dc);
 ...
 */
void MtuNetHelper::SetQueue(std::string type, std::string n1, const AttributeValue &v1, std::string n2, const AttributeValue &v2, std::string n3, const AttributeValue &v3,
